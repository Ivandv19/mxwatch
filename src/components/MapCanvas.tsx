...
import { useSearchQuery, useSelectedCartel, useSelectedState, useLiveStateData, useMapActions } from "@/store/mapStore";
...
export default function MapCanvas() {
    ...
    useEffect(() => {
        ...
        fetch(geoUrl, { signal: controller.signal })
            ...
            .then(data => { setTopoData(data); setErrors(prev => ({ ...prev, map: null })); })
            .catch(err => err.name !== 'AbortError' && setErrors(prev => ({ ...prev, map: "Error geometría" })))
            .finally(() => setIsLoading(prev => ({ ...prev, map: false })));

        // 2. Carga de Inteligencia en Vivo
        setIsLoading(prev => ({ ...prev, data: true }));
        getLiveMapData()
            .then(data => { setLiveStateData(data); setErrors(prev => ({ ...prev, data: null })); })
            .catch(() => setErrors(prev => ({ ...prev, data: "Error inteligencia" })))
    ...
    const handleZoomIn = useCallback(() => setPosition(p => ({ ...p, zoom: Math.min(p.zoom * ZOOM_STEP, MAX_ZOOM) })), []);
    const handleZoomOut = useCallback(() => setPosition(p => ({ ...p, zoom: Math.max(p.zoom / ZOOM_STEP, DEFAULT_ZOOM) })), []);
    const handleReset = useCallback(() => setPosition({ coordinates: MEXICO_CENTER, zoom: DEFAULT_ZOOM }), []);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (['+', '='].includes(e.key)) { e.preventDefault(); handleZoomIn(); }
            if (['-', '_'].includes(e.key)) { e.preventDefault(); handleZoomOut(); }
            if (['r', 'R', '0'].includes(e.key)) { e.preventDefault(); handleReset(); }
    ...
    const getCartelStyle = useCallback((stateName: string) => {
        ...
        if (!stateRecord || stateRecord.cartels.length === 0) {
            return { fill: "rgba(25, 40, 60, 0.4)", stroke: "rgba(80, 110, 150, 0.4)", strokeWidth: 0.5, cartel: "Sin datos", opacity: 1, liveDataRaw: null };
        }

        const isCartelMatch = selectedCartel && stateRecord.cartels.some((c: any) => c.slug === selectedCartel || c.id === selectedCartel);
        ...
        if (stateRecord.cartels.length > 1 && stateName === "Tamaulipas") {
            return {
                fill: isDimmed ? "url(#pattern-tamaulipas-dimmed)" : isCartelMatch ? "url(#pattern-tamaulipas-highlighted)" : "url(#pattern-tamaulipas-normal)",
                stroke: isDimmed ? `${primaryColor}30` : isCartelMatch ? "white" : primaryColor,
                strokeWidth: isCartelMatch ? 2 : 1,
                cartel: cartelNames, opacity: isDimmed ? 0.4 : 1, liveDataRaw: dominantCartel
            };
        }

        // Estilo estándar por cartel dominante
        return {
            fill: isDimmed ? `${primaryColor}18` : isCartelMatch ? `${primaryColor}dd` : `${primaryColor}44`,
            stroke: isDimmed ? `${primaryColor}30` : isCartelMatch ? "white" : primaryColor,
            strokeWidth: isCartelMatch ? 2 : 1,
            cartel: cartelNames, opacity: isDimmed ? 0.4 : 1, liveDataRaw: dominantCartel
        };
    ...
    const features = useMemo(() => {
        ...
        try { return (feature(topoData, topoData.objects.states) as any).features; }
        catch { return []; }
    }, [topoData]);

    // Escala adaptativa según dispositivo (Mobile/Desktop)
    const mapScale = useMemo(() => typeof window !== 'undefined' && window.innerWidth < 768 ? 900 : 1400, []);

    return (
        <div ref={mapContainerRef} className="relative flex-1 w-full h-full bg-[#080c12] overflow-hidden cursor-crosshair">
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-[#05080c]" style={{ backgroundImage: `radial-gradient(circle, rgba(255, 255, 255, 0.15) 1px, transparent 1px)` }}>
                {tooltip && <StrategicTooltip tooltip={tooltip} />}

                {!topoData && !errors.map ? <LoadingIndicator /> : topoData && (
                    <MemoizedMap
                        features={features} position={position} handleMoveEnd={setPosition}
                        getCartelStyle={getCartelStyle} selectedState={selectedState}
                        setSelectedState={setSelectedState} setTooltip={setTooltip}
                        mapScale={mapScale} containerRef={mapContainerRef}
                    />
                )}

                <KeyboardShortcutsHint />
                <MapControls onZoomIn={handleZoomIn} onZoomOut={handleZoomOut} onReset={handleReset} zoom={position.zoom} />
            </div>
        </div>
    );
}
...
const StrategicTooltip = React.memo(({ tooltip }: { tooltip: TooltipState }) => (
    <div className="absolute z-50 pointer-events-none transition-transform duration-75 ease-out" style={{ left: tooltip.x + 15, top: tooltip.y + 15 }}>
        <div className="bg-[#0a0f18]/95 backdrop-blur-md border border-white/10 rounded-lg p-3 shadow-2xl flex flex-col gap-1.5 min-w-[180px]">
            ...
            <span className="text-sm font-bold text-[#f0f4ff]">{tooltip.content}</span>
...
const LoadingOverlay = () => (
    <div className="absolute inset-0 bg-[#080c12]/80 backdrop-blur-sm flex items-center justify-center z-40">
        <div className="flex flex-col items-center gap-4">
            ...
            <span className="text-xs font-mono uppercase tracking-[0.2em] text-[#8b98b8]">Sincronizando...</span>
        </div>
    </div>
);
...
const MapControls = React.memo(({ onZoomIn, onZoomOut, onReset, zoom }: any) => (
    <div className="absolute top-20 md:bottom-6 right-4 md:right-6 flex flex-col gap-2 z-20">
        <button onClick={onZoomIn} disabled={zoom >= MAX_ZOOM} className="w-9 h-9 rounded-lg bg-[#0f1520]/80 border border-white/10 hover:bg-[#1c2636] transition-all disabled:opacity-30" aria-label="Acercar">+</button>
        <button onClick={onZoomOut} disabled={zoom <= DEFAULT_ZOOM} className="w-9 h-9 rounded-lg bg-[#0f1520]/80 border border-white/10 hover:bg-[#1c2636] transition-all disabled:opacity-30" aria-label="Alejar">-</button>
        <button onClick={onReset} className="w-9 h-9 rounded-lg bg-[#0f1520]/80 border border-white/10 text-accent hover:bg-accent/10 transition-all" aria-label="Resetear">⟲</button>
    </div>
));
...
const KeyboardShortcutsHint = () => (
    <div className="absolute bottom-6 left-6 z-30 bg-[#0f1520]/60 backdrop-blur-sm border border-white/5 rounded-lg px-3 py-2 text-[10px] font-mono text-[#5e6c8b] hidden md:block">
        <div className="flex items-center gap-3"><span>+ / - : Zoom</span><span>R : Reset</span><span>ESC : Limpiar</span></div>
    </div>
);
...
const ErrorDisplay = ({ message, type }: { message: string; type: 'map' | 'data' }) => (
    <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-50">
        <div className="bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-2 flex items-center gap-2">
            ...
            <button onClick={() => window.location.reload()} className="text-xs text-red-400/70 hover:text-red-400 ml-2 underline">Reintentar</button>
        </div>
    </div>
);
 ...
 */
const MemoizedMap = React.memo(({ features, position, handleMoveEnd, getCartelStyle, selectedState, setSelectedState, setTooltip, mapScale, containerRef }: any) => {
    const handleMouseEnter = useCallback((e: React.MouseEvent, stateName: string, style: any) => {
        if (!containerRef.current) return;
        const rect = containerRef.current.getBoundingClientRect();
        setTooltip({ content: stateName, cartel: style.cartel, color: style.stroke === "white" ? (style.liveDataRaw?.color ?? "#8b98b8") : style.stroke, x: e.clientX - rect.left, y: e.clientY - rect.top });
    ...
    return (
        <div className="absolute inset-0 z-10">
            <ComposableMap projection="geoMercator" projectionConfig={{ scale: mapScale, center: MEXICO_CENTER }} className="w-full h-full">
                <defs>
                    ...
                    <pattern id="pattern-tamaulipas-normal" width="12" height="12" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                        <rect width="6" height="12" fill="#9b59b6" fillOpacity={0.4} /><rect x="6" width="6" height="12" fill="#1abc9c" fillOpacity={0.4} />
                    </pattern>
                    <pattern id="pattern-tamaulipas-highlighted" width="12" height="12" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                        <rect width="6" height="12" fill="#9b59b6" fillOpacity={0.8} /><rect x="6" width="6" height="12" fill="#1abc9c" fillOpacity={0.8} />
                    </pattern>
                    <pattern id="pattern-tamaulipas-dimmed" width="12" height="12" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                        <rect width="6" height="12" fill="#9b59b6" fillOpacity={0.15} /><rect x="6" width="6" height="12" fill="#1abc9c" fillOpacity={0.15} />
                    </pattern>
                </defs>

                <ZoomableGroup zoom={position.zoom} center={position.coordinates} onMoveEnd={handleMoveEnd} maxZoom={MAX_ZOOM}>
                    <Geographies geography={features}>
                        {({ geographies }) => geographies.map((geo) => {
                            const stateName = geo.properties.state_name;
                            const style = getCartelStyle(stateName);
                            return (
                                <Geography key={geo.rsmKey} geography={geo} fill={style.fill} stroke={style.stroke} strokeWidth={style.strokeWidth}
                                    onMouseEnter={(e) => handleMouseEnter(e, stateName, style)} onMouseMove={handleMouseMove} onMouseLeave={() => setTooltip(null)}
                                    onClick={() => setSelectedState(selectedState === stateName ? null : stateName)}
                                    style={{ default: { opacity: style.opacity, outline: "none", transition: "all 200ms ease" }, hover: { fillOpacity: 0.8, stroke: "white", strokeWidth: 2 } }}
                                />
                            );
                        })}
                    </Geographies>
...
}, (prev, next) => prev.position.zoom === next.position.zoom && prev.selectedState === next.selectedState && prev.features === next.features);
