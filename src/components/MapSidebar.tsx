...
import { getAllCartelsBasic, getStateIntelligence } from "@/actions/mapData";
import {
	useLiveStateData,
	useMapActions,
	useSearchQuery,
...
const SearchIcon = () => (
	<svg
		className="w-4 h-4 text-[#5e6c8b] absolute left-3 top-1/2 -translate-y-1/2"
		fill="none"
		viewBox="0 0 24 24"
		stroke="currentColor"
		aria-hidden="true"
...
export default function MapSidebar() {
	...
	const [stateIntelligence, setStateIntelligence] =
		useState<LiveStateIntelligence | null>(null);
	const [isLoadingIntelligence, setIsLoadingIntelligence] = useState(false);
	const [intelligenceError, setIntelligenceError] = useState<string | null>(
		null,
	);
	const [allCartels, setAllCartels] = useState<BasicCartel[]>([]);
	...
	useEffect(() => {
		getAllCartelsBasic()
			...
			.catch((err) => {
				...
				setCartelsError("Fallo al cargar listado");
			});
	}, []);
	...
	useEffect(() => {
		...
		getStateIntelligence(selectedState)
			.then((data) => {
				if (!data) {
					setIntelligenceError("Sin datos para este estado");
					return;
				}
			...
			.catch((err) => {
				...
				setIntelligenceError("Error de conexión");
			})
	...
	const handleRetryCartels = useCallback(() => {
		...
		getAllCartelsBasic()
			.then(setAllCartels)
			.catch(() => setCartelsError("Fallo al reintentar"));
	}, []);
	...
	const handleRetryIntelligence = useCallback(() => {
		...
		setIntelligenceError(null);
		getStateIntelligence(selectedState)
			.then(setStateIntelligence)
			.catch(() => setIntelligenceError("Fallo al reintentar"))
			.finally(() => setIsLoadingIntelligence(false));
	}, [selectedState]);
		...
		<aside className="w-full h-[40%] md:h-full md:w-[380px] bg-[#0f1520] border-b md:border-b-0 md:border-r border-white/10 flex flex-col shrink-0 overflow-hidden z-10 shadow-2xl">
			...
			<div className="flex-1 overflow-y-auto overflow-x-hidden">
				...
				<div className="p-4 flex flex-col gap-6">
							...
							>
								{isLoadingIntelligence ? (
									<div className="rounded-xl border border-white/10 bg-white/5 p-8 flex flex-col items-center justify-center">
										...
										<span className="text-xs text-[#8b98b8] font-mono uppercase tracking-[0.2em]">
											Leyendo expediente...
										</span>
									</div>
								) : intelligenceError ? (
									<ErrorAlert
										...
										onRetry={handleRetryIntelligence}
									/>
								) : stateIntelligence ? (
									<SelectedStatePanel
					...
					<QuickStats
						stateCount={liveStateData.length}
						cartelsCount={allCartels.length}
					/>
		...
		>
			<div className="flex items-center justify-between">
				...
				<button
					type="button"
					onClick={onClear}
					className="text-[#5e6c8b] hover:text-[#f0f4ff] text-xs"
				>
					✕
				</button>
			</div>
			...
			<div className="flex flex-col gap-6">
				...
				{stateIntelligence.cartels.map((cartelInfo, idx) => (
...
const CartelDetail = React.memo(
	({
		...
		cartel: LiveStateIntelligence["cartels"][number];
		index: number;
	}) => (
		<motion.div
		...
		>
			{index > 0 && (
				<div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0f1520] px-2 text-[8px] text-[#5e6c8b] uppercase font-bold">
			...
			<div className="flex flex-col gap-4">
				<div className="flex flex-wrap gap-2">
					...
					{cartel.foreignDesignation && (
						<span className="text-[9px] font-black uppercase bg-red-900/30 text-red-400 px-2 py-0.5 rounded border border-red-500/30">
							{cartel.foreignDesignation}
						</span>
					)}
				</div>
				...
				{cartel.leaders?.length > 0 && (
					<div className="flex flex-col gap-1">
						<span className="text-[9px] uppercase tracking-widest text-[#5e6c8b] font-bold">
							Cadena de Mando Regional
						</span>
						{cartel.leaders.map((l) => (
							<span key={l.name} className="text-xs text-[#f0f4ff] font-medium">
								{l.name} {l.alias ? `("${l.alias}")` : ""}
							</span>
						))}
					</div>
				)}
	...
	({
		...
		<div className="grid grid-cols-2 gap-3">
			<div className="bg-[#161d2b] border border-white/5 rounded-lg p-3 flex flex-col py-4">
				<span className="text-[10px] uppercase font-bold text-[#5e6c8b] mb-1">
					Cárteles
				</span>
...
const CartelLegend = React.memo(
	({
		...
		onToggleCartel: (slug: string) => void;
		...
		return (
			<div className="flex flex-col gap-3">
				...
				<div className="flex flex-col gap-1.5">
					{cartels.map((cartel) => {
						const isSelected =
							selectedCartel === cartel.id || selectedCartel === cartel.slug;
						const count =
							(stateCountByCartel[cartel.slug] ?? 0) ||
							(stateCountByCartel[cartel.id] ?? 0);
						return (
							<button
								type="button"
								key={cartel.id}
								onClick={() => onToggleCartel(cartel.slug || cartel.id)}
								className={`flex items-center gap-3 p-2 rounded-md transition-all text-left w-full group ${isSelected ? "bg-[#1c2636] border border-accent/30" : "hover:bg-[#161d2b] border border-transparent"} ${selectedCartel && !isSelected ? "opacity-30" : "opacity-100"}`}
							>
								<div
									className="w-3 h-3 rounded-sm shadow-sm"
									style={{ backgroundColor: cartel.color }}
								/>
								<span className="text-sm font-medium flex-1 truncate text-[#f0f4ff]">
									{cartel.name}
								</span>
								<span
...
const SidebarFooter = React.memo(() => (
	<div className="p-4 border-t border-white/5 mt-auto flex flex-col items-center gap-2">
		...
		<span className="text-[9px] text-[#5e6c8b] uppercase tracking-widest opacity-40">
