...
// Constantes de UI para cálculos de layout
const CARTEL_ITEM_HEIGHT = 48;
...
const ClearButton = ({ onClick }: { onClick: () => void }) => (
    <button onClick={onClick} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5e6c8b] hover:text-[#f0f4ff] transition-colors p-0.5" aria-label="Limpiar búsqueda">
...
/** Alerta de error estilizada con opción de reintento */
const ErrorAlert = ({ message, onRetry }: { message: string; onRetry?: () => void }) => (
    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 text-center mx-4">
        <span className="text-xs text-red-400">⚠️ {message}</span>
        {onRetry && (
            <button onClick={onRetry} className="block mx-auto mt-2 text-xs text-accent hover:text-accent-hover transition-colors">Reintentar</button>
        )}
    </motion.div>
);

/**
 * Panel lateral de inteligencia: gestión de filtros, selección de estados y persistencia en URL.
 */
export default function MapSidebar() {
    ...
    // Estado Local: Datos de inteligencia y listado maestro
    ...
    /** Efecto: Sincronización inicial Store → URL al montar el componente */
    useEffect(() => {
    ...
    /** Efecto: Persistencia de interacciones en la URL (Search Params) */
    useEffect(() => {
    ...
    /** Efecto: Atajos de teclado para navegación y limpieza de filtros */
    useEffect(() => {
            ...
            switch (e.key) {
                ...
                case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9':
                    if (e.altKey && allCartels.length > 0) {
                        ...
                        if (allCartels[index]) toggleCartel(allCartels[index].slug || allCartels[index].id);
                    }
                    break;
            }
        };
    ...
    /** Efecto: Carga del listado maestro de cárteles */
    useEffect(() => {
        getAllCartelsBasic()
            .then(data => { setAllCartels(data); setCartelsError(null); })
            .catch(err => { console.error("Error loading cartels:", err); setCartelsError("Fallo al cargar listado"); });
    }, []);

    /** Efecto: Carga de inteligencia detallada al seleccionar un estado */
    useEffect(() => {
        if (!selectedState) { setStateIntelligence(null); setIntelligenceError(null); return; }
        ...
        getStateIntelligence(selectedState)
            .then(data => {
                if (!data) { setIntelligenceError("Sin datos para este estado"); return; }
                setStateIntelligence(data);
            })
            .catch(err => { console.error("Error loading intelligence:", err); setIntelligenceError("Error de conexión"); })
            .finally(() => setIsLoadingIntelligence(false));
    }, [selectedState]);

    /** Memoización: Conteo de presencia territorial por cártel */
    ...
    /** Memoización: Color primario del estado seleccionado para UI dinámica */
    const primaryColor = useMemo(
        () => stateIntelligence?.cartels[0]?.color || null,
        [stateIntelligence]
    );

    /** Memoización: Filtrado de cárteles según búsqueda textual */
    const filteredCartels = useMemo(() => {
        if (!searchQuery) return allCartels;
        return allCartels.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [allCartels, searchQuery]);

    // Handlers de reintento para errores de carga
    ...
    return (
        <aside className="w-full h-[40%] md:h-full md:w-[380px] bg-[#0f1520] border-b md:border-b-0 md:border-r border-white/10 flex flex-col shrink-0 overflow-hidden z-10 shadow-2xl">
            ...
            {/* Buscador Superior con controles de ayuda */}
            <div className="p-3 md:p-4 border-b border-white/10 bg-[#080c12]/50 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center justify-between mb-2 md:mb-3">
                    <h2 className="text-[10px] md:text-sm font-bold tracking-widest uppercase text-[#f0f4ff]">Centro de Análisis</h2>
                    <InfoTooltip content="Atajos: Foco (Ctrl+F), ESC limpiar, Alt+1-9 selección rápida" />
                </div>
                <div className="relative">
                    <input type="text" placeholder="Buscar estado o cártel..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#161d2b] border border-white/20 rounded-lg pl-10 pr-10 py-2 text-sm text-[#f0f4ff] placeholder:text-[#5e6c8b] focus:outline-none focus:border-accent transition-colors" />
                    <SearchIcon />
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden">
                <div className="flex border-b border-white/5 bg-[#0f1520] sticky top-0 z-10">
                    ...
                    <button disabled className="flex-1 py-3 text-xs font-bold uppercase tracking-wide text-[#5e6c8b] opacity-30 cursor-not-allowed">Incidentes</button>
                </div>

                <div className="p-4 flex flex-col gap-6">
                    {/* Panel de Inteligencia del Estado Seleccionado */}
                    <AnimatePresence mode="wait">
                        {selectedState && (
                            <motion.div key={selectedState} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                                {isLoadingIntelligence ? (
                                    ...
                                    <SelectedStatePanel selectedState={selectedState} stateIntelligence={stateIntelligence} primaryColor={primaryColor} onClear={() => setSelectedState(null)} />
                                ) : null}
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    {/* Leyenda Interactiva de Cárteles */}
                    {cartelsError ? (
                        <ErrorAlert message={cartelsError} onRetry={handleRetryCartels} />
                    ) : (
                        <CartelLegend cartels={filteredCartels} selectedCartel={selectedCartel} onToggleCartel={toggleCartel} stateCountByCartel={stateCountByCartel} />
                    )}
                </div>
            </div>
...
/** Panel de detalles del estado seleccionado con análisis táctico */
const SelectedStatePanel = React.memo(({ selectedState, stateIntelligence, primaryColor, onClear }: any) => (
    <div className="rounded-xl border p-4 flex flex-col gap-3 transition-all" style={{ borderColor: primaryColor ? `${primaryColor}40` : "rgba(255,255,255,0.1)", backgroundColor: primaryColor ? `${primaryColor}10` : "rgba(255,255,255,0.03)" }}>
...
/** Visualización granular de un cártel: líderes, facciones y estatus */
const CartelDetail = React.memo(({ cartel, index, isDisputed }: any) => (
    <motion.div initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className={`flex flex-col gap-3 ${index > 0 ? "pt-6 border-t border-white/10 relative" : ""}`}>
        ...
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap gap-2">
                <span className="text-[10px] font-black uppercase bg-accent/20 text-accent px-2 py-0.5 rounded border border-accent/30">{cartel.localIntelligenceNote || "Operativo Activo"}</span>
                {cartel.foreignDesignation && <span className="text-[9px] font-black uppercase bg-red-900/30 text-red-400 px-2 py-0.5 rounded border border-red-500/30">{cartel.foreignDesignation}</span>}
            </div>
            ...
            {cartel.leaders?.length > 0 && (
                <div className="flex flex-col gap-1">
                    ...
                    {cartel.leaders.map((l: any, i: number) => <span key={i} className="text-xs text-[#f0f4ff] font-medium">{l.name} {l.alias ? `("${l.alias}")` : ''}</span>)}
                </div>
            )}
...
/** Contadores rápidos de la base de datos (KPIs) */
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
                {cartels.map((cartel: any) => {
                    ...
                    return (
                        <button key={cartel.id} onClick={() => onToggleCartel(cartel.slug || cartel.id)} className={`flex items-center gap-3 p-2 rounded-md transition-all text-left w-full group ${isSelected ? "bg-[#1c2636] border border-accent/30" : "hover:bg-[#161d2b] border border-transparent"} ${selectedCartel && !isSelected ? "opacity-30" : "opacity-100"}`}>
...
/** Pie de firma del sistema con versión y fecha de actualización */
